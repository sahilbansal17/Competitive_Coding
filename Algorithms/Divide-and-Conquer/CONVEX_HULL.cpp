// Program to find Convex
// Hull of a given set of points using Divide and Conquer Approach.
...
int main()
{
    ...
    cout<< "Enter the x any y coordinates of all points: "<< endl;
    for (int i = 1; i <= n; i++)
    {
        cin>> x>> y;
...
// Time Complexity: O(n * log(n)) because merge operation takes O(n) time
// because we are dividing the points into two equal parts.
// O(log(n)) time is required for dividing the points into two eqaul parts.
